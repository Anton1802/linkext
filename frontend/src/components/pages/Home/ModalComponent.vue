<template>
    <n-modal v-model:show="store.showModal">
        <n-card style="width: 600px" title="Shorten URL" :bordered="false" size="huge" role="dialog" aria-modal="true">
            <p style="font-size: 18px; margin: 0; padding: 0;">Full URL: <a :href="store.originalUrl ? store.originalUrl : '#'">{{
                    store.originalUrl }}</a></p>
            <n-form :size="'large'">
                <n-form-item>
                    <n-input style="text-align: start; margin-right: 2px;" :autofocus="true" readonly="true" placeholder="" status="error" :value="store.shortenLink"/>
